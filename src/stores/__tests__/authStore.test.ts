...
describe("AuthStore", () => {
	...
	describe("setupAuth", () => {
		...
		it("should setup biometric authentication successfully", async () => {
			...
			(biometricAuthService.getCapabilities as any).mockResolvedValue({
				available: true,
				enrolled: true,
			});
			(biometricAuthService.setup as any).mockResolvedValue({
				success: true,
				method: "biometric",
			});

			const store = useAuthStore.getState();
			const result = await store.setupAuth("biometric", {
	...
	describe("authenticate", () => {
		...
		it("should authenticate with biometric successfully", async () => {
			(biometricAuthService.authenticate as any).mockResolvedValue(
				mockSuccessResult,
			);
