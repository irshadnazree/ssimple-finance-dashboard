...
export const useAuthStore = create<AuthStoreState>((set, get) => ({
		...
		try {
			...
			const authServiceProxy = {
				...
				addEventListener: (
					event: string,
					callback: (event: AuthEvent) => void,
				) => get().addEventListener(event, callback),
	...
	authenticatePin: async (pin: string) => {
		try {
			const storedCredentials = localStorage.getItem(
				AUTH_STORAGE_KEYS.PIN_CREDENTIALS,
			);
	...
	lockAccount: () => {
		const state = get();
		const lockedUntil =
			Date.now() + state.authState.config.lockoutDuration * 60 * 1000;
	...
	saveFailedAttempt: (attempt: AuthAttempt) => {
		try {
			...
			const failedCount = recentAttempts.filter(
				(a) => !a.success && Date.now() - a.timestamp < 24 * 60 * 60 * 1000, // Last 24 hours
			).length;
