...
export interface UIActions {
  ...
  addNotification: (notification: Omit<UIState['notifications']['notifications'][0], 'id' | 'timestamp' | 'isRead'>) => void;
    ...
    getFilteredTransactions: (transactions) => {
      ...
      return transactions.filter(transaction => {
        ...
        if (filters.searchQuery) {
          ...
          const searchableText = `${transaction.description} ${transaction.category} ${transaction.note || ''}`.toLowerCase();
