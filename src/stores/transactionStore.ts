...
interface TransactionState {
  ...
  // Bulk operations
  bulkCreateTransactions: (transactions: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>[]) => Promise<BulkTransactionResult>;
  bulkUpdateTransactions: (updates: Array<{ id: string; updates: Partial<Transaction> }>) => Promise<BulkTransactionResult>;
  ...
  getCategoryBreakdown: (type?: Transaction['type']) => Record<string, { amount: number; count: number }>;
  ...
  // Import/Export
  importTransactions: (data: string, format?: 'json' | 'csv') => Promise<BulkTransactionResult>;
  exportTransactions: (transactions: Transaction[], format?: 'json' | 'csv') => string;
  ...
  validateTransaction: (transaction: Partial<Transaction>) => Promise<ValidationError[]>;
  suggestCategory: (description: string, amount: number) => Promise<string | null>;
}
...
export const useTransactionStore = create<TransactionState>((set, get) => ({
  ...
  createTransaction: async (transaction: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      ...
      const account = get().accounts.find(a => a.id === newTransaction.account);
      if (!account) {
        throw new Error('Account not found');
      }
      
      const category = get().categories.find(c => c.id === newTransaction.category);
  ...
  bulkCreateTransactions: async (transactions: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>[]) => {
    set({ isLoading: true, error: null });
    try {
      const state = get();
  ...
  bulkUpdateTransactions: async (updates: Array<{ id: string; updates: Partial<Transaction> }>) => {
    set({ isLoading: true, error: null });
    try {
      const state = get();
      const updatedTransactions = [...state.transactions];
  ...
  getTransactionSummary: (filters?: TransactionFilters) => {
    ...
    const summary: TransactionSummary = {
      ...
      accountBreakdown: {},
      period: {
        startDate: filters?.startDate || new Date(Math.min(...transactions.map(t => t.date.getTime()))),
        endDate: filters?.endDate || new Date(Math.max(...transactions.map(t => t.date.getTime())))
      }
    };
  ...
  searchTransactions: (query: string) => {
    ...
    return transactions.filter(transaction => 
      ...
      (transaction.tags?.some(tag => tag.toLowerCase().includes(lowercaseQuery)) ?? false)
    );
  },
  ...
  importTransactions: async (data: string, format = 'json') => {
    ...
    try {
      ...
      for (const transaction of transactions) {
        try {
          ...
          if (validationErrors.length > 0) {
            result.failed++;
            result.errors.push(`Transaction validation failed: ${validationErrors.map(e => e.message).join(', ')}`);
            continue;
          }
          
          await get().createTransaction(transaction as Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>);
          result.processed++;
        } catch (error) {
          result.failed++;
      ...
      result.errors.push(error instanceof Error ? error.message : 'Failed to parse import data');
      return result;
    }
  },
  ...
  exportTransactions: (transactions: Transaction[], format = 'json') => {
    ...
    // CSV export
    const headers = ['ID', 'Date', 'Description', 'Amount', 'Type', 'Category', 'Account'];
  ...
  validateTransaction: async (transaction: Partial<Transaction>) => {
    ...
    if (!transaction.account) {
      errors.push({ field: 'account', message: 'Account is required', code: 'REQUIRED_FIELD' });
    }
