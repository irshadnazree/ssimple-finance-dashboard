...
import type { Transaction, ReportFilters } from '../../types/finance';
...
export function useReportPerformance(transactions: Transaction[]) {
  ...
  const [performanceStats, setPerformanceStats] = useState<Record<string, unknown> | null>(null);
...
export function usePerformanceMonitor(componentName: string) {
  ...
  const logPerformance = useCallback(() => {
    console.log(`${componentName} - Renders: ${renderCountRef.current}, Last render time: ${renderTime.toFixed(2)}ms`);
