...
export class ExportService {
  ...
  async exportFinancialSummary(
    ...
    const filename = options.filename || `financial-summary-${format(new Date(), 'yyyy-MM-dd')}`;
  ...
  async exportTransactionAnalysis(
    ...
    const filename = options.filename || `transaction-analysis-${format(new Date(), 'yyyy-MM-dd')}`;
  ...
  async exportCategoryBreakdown(
    ...
    const filename = options.filename || `category-breakdown-${format(new Date(), 'yyyy-MM-dd')}`;
  ...
  async exportTransactions(
    ...
    const filename = options.filename || `transactions-${format(new Date(), 'yyyy-MM-dd')}`;
  ...
  // CSV Export Methods
  private async exportFinancialSummaryCSV(report: FinancialSummaryReport, filename: string): Promise<void> {
    const csvContent = [
      ['Metric', 'Value'],
      ...
      ['Average Daily Spending', report.summary.averageDailySpending.toString()],
      ['Largest Expense', `${report.summary.largestExpense.description}: $${report.summary.largestExpense.amount}`],
      ['Largest Income', `${report.summary.largestIncome.description}: $${report.summary.largestIncome.amount}`]
    ];
    ...
    if (report.periodComparison) {
      csvContent.push(
        ['', ''],
        ['Period Comparison', ''],
        ['Previous Income', report.periodComparison.previousPeriod.income.toString()],
        ['Previous Expenses', report.periodComparison.previousPeriod.expenses.toString()],
        ['Previous Net Income', report.periodComparison.previousPeriod.netIncome.toString()],
        ['Income Change %', `${report.periodComparison.changePercentage.income.toFixed(2)}%`],
        ['Expenses Change %', `${report.periodComparison.changePercentage.expenses.toFixed(2)}%`],
        ['Net Income Change %', `${report.periodComparison.changePercentage.netIncome.toFixed(2)}%`]
      );
    }
  ...
  private async exportTransactionAnalysisCSV(report: TransactionAnalysisReport, filename: string): Promise<void> {
    const csvContent = [
      ['Transaction Statistics', ''],
      ['Total Transactions', report.transactionStats.totalTransactions.toString()],
      ['Average Amount', report.transactionStats.averageTransactionAmount.toString()],
      ['Median Amount', report.transactionStats.medianTransactionAmount.toString()],
      ['Daily Frequency', report.transactionStats.transactionFrequency.daily.toString()],
      ['Weekly Frequency', report.transactionStats.transactionFrequency.weekly.toString()],
      ['Monthly Frequency', report.transactionStats.transactionFrequency.monthly.toString()],
      ['', ''],
      ['Largest Expenses', ''],
  ...
  private async exportCategoryBreakdownCSV(report: CategoryBreakdownReport, filename: string): Promise<void> {
    const csvContent = [
      ['Income Categories', ''],
      ['Category', 'Amount', 'Percentage', 'Transaction Count', 'Average Amount'],
  ...
  private async exportTransactionsCSV(transactions: Transaction[], filename: string): Promise<void> {
    const csvContent = [
  ...
  private async exportFinancialSummaryExcel(report: FinancialSummaryReport, filename: string): Promise<void> {
  ...
  private async exportTransactionAnalysisExcel(report: TransactionAnalysisReport, filename: string): Promise<void> {
    await this.exportTransactionAnalysisCSV(report, filename);
    console.warn('Excel export not fully implemented. Exported as CSV format.');
  }

  private async exportCategoryBreakdownExcel(report: CategoryBreakdownReport, filename: string): Promise<void> {
    await this.exportCategoryBreakdownCSV(report, filename);
    console.warn('Excel export not fully implemented. Exported as CSV format.');
  }

  private async exportTransactionsExcel(transactions: Transaction[], filename: string): Promise<void> {
    await this.exportTransactionsCSV(transactions, filename);
    console.warn('Excel export not fully implemented. Exported as CSV format.');
  }
  ...
  private generateFinancialSummaryHTML(report: FinancialSummaryReport, options: ExportOptions): string {
  ...
  private generateTransactionAnalysisHTML(report: TransactionAnalysisReport, options: ExportOptions): string {
    const dateRange = options.dateRange 
      ? `${format(options.dateRange.start, 'MMM dd, yyyy')} - ${format(options.dateRange.end, 'MMM dd, yyyy')}`
      : 'All Time';
    ...
    return `
      ...
      <body>
        ...
        <table>
          ...
          <tbody>
            ${report.topTransactions.largestExpenses.concat(report.topTransactions.largestIncomes).map(transaction => `
              <tr>
  ...
  private generateCategoryBreakdownHTML(report: CategoryBreakdownReport, options: ExportOptions): string {
    const dateRange = options.dateRange 
      ? `${format(options.dateRange.start, 'MMM dd, yyyy')} - ${format(options.dateRange.end, 'MMM dd, yyyy')}`
      : 'All Time';
    ...
    return `
      ...
      <body>
        ...
        <table>
          ...
          <tbody>
            ${report.incomeCategories.concat(report.expenseCategories).map(category => `
              <tr>
  ...
  private generateTransactionsHTML(transactions: Transaction[], options: ExportOptions): string {
    const dateRange = options.dateRange 
      ? `${format(options.dateRange.start, 'MMM dd, yyyy')} - ${format(options.dateRange.end, 'MMM dd, yyyy')}`
      : 'All Time';
