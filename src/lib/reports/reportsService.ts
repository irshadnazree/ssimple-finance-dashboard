...
export class ReportsService {
	...
	async generateTransactionAnalysisReport(
		filters: ReportFilters,
		...
		for (const t of transactions) {
			const existing = categoryStats.get(t.category) || {
				count: 0,
				totalAmount: 0,
			};
	...
	async generateCategoryBreakdownReport(
		filters: ReportFilters,
		...
		// Generate chart data
		const chartData = [...expenseCategories, ...incomeCategories].map(
			(cat) => ({
	...
	async generateAccountPerformanceReport(
		filters: ReportFilters,
	): Promise<AccountPerformanceReport> {
		const accounts = await DatabaseService.getAccounts();
		const transactions = useTransactionStore.getState().getTransactions({
			startDate: filters.dateRange.startDate,
			endDate: filters.dateRange.endDate,
		...
		const accountPerformance = await Promise.all(
			accounts.map(async (account) => {
				...
				const performanceScore = Math.min(
					100,
					Math.max(0, 50 + changePercentage * 2 + transactionCount * 0.1),
				);
		...
		const netWorthChange = accountPerformance.reduce(
			(sum, a) => sum + a.netChange,
			0,
		);
	...
	async generateCashFlowReport(
		...
		const monthlyInflows =
			cashFlowData.reduce((sum, d) => sum + d.inflows, 0) /
			(cashFlowData.length / 30 || 1);
		const monthlyOutflows =
			cashFlowData.reduce((sum, d) => sum + d.outflows, 0) /
			(cashFlowData.length / 30 || 1);

		// Calculate volatility (standard deviation of daily net flows)
		const netFlows = cashFlowData.map((d) => d.netFlow);
		const avgNetFlow =
			netFlows.reduce((sum, f) => sum + f, 0) / netFlows.length;
		const variance =
			netFlows.reduce((sum, f) => sum + (f - avgNetFlow) ** 2, 0) /
			netFlows.length;
		...
		for (const d of cashFlowData) {
			if (d.netFlow > 0) {
				...
				longestNegativeStreak = Math.max(
	...
	private processCategoryBreakdown(
		...
		const categoryStats = new Map<
			string,
			{ amount: number; count: number; transactions: Transaction[] }
		>();
