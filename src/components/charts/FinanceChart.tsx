...
export default function FinanceChart({
	...
	const loadChartData = useCallback(async () => {
		try {
			...
			const formattedData = groupedData.map((item) => ({
				...item,
				date: new Intl.DateTimeFormat("en-US", dateFormat).format(
					new Date(item.date),
				),
			}));
			...
			<Card className={className}>
				<CardContent className="p-6">
					<div className="text-center text-destructive">
						...
						<Button onClick={loadChartData} className="mt-2" variant="default">
							Retry
						</Button>
					</div>
				</CardContent>
	...
	return (
		<Card className={className}>
			<CardHeader>
				<CardTitle>Income vs Expenses ({timeframe})</CardTitle>
			</CardHeader>
			<CardContent>
				{chartData.length === 0 ? (
						...
						<div className="relative h-64 border-l border-b border-border">
							<div className="absolute inset-0 flex items-end justify-around px-2">
								{chartData.map((dataPoint) => {
									const incomeHeight =
										maxValue > 0 ? (dataPoint.income / maxValue) * 100 : 0;
									const expenseHeight =
										maxValue > 0 ? (dataPoint.expenses / maxValue) * 100 : 0;
