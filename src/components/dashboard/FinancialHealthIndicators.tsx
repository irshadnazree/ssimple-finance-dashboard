...
export function FinancialHealthIndicators({ className = "", healthData }: FinancialHealthIndicatorsProps) {
	// Calculate health status based on common financial health metrics
	const getHealthStatus = (value: number, thresholds: { excellent: number; good: number; warning: number }) => {
	...
	const metrics: HealthMetric[] = [
		{
			...
			unit: '%',
			status: getHealthStatus(healthData.savingsRate, { excellent: 20, good: 15, warning: 10 }),
			icon: TrendingUp,
		...
		},
		{
			...
			unit: '%',
			status: getHealthStatus(100 - healthData.debtToIncomeRatio, { excellent: 64, good: 50, warning: 30 }),
			icon: AlertTriangle,
		...
		},
		{
			...
			unit: ' months',
			status: getHealthStatus(healthData.emergencyFundMonths, { excellent: 6, good: 4, warning: 2 }),
			icon: CheckCircle,
		...
		},
		{
			...
			unit: '%',
			status: getHealthStatus(100 - healthData.monthlyBudgetUsage, { excellent: 20, good: 10, warning: 5 }),
			icon: Target,
	...
	const overallScore = metrics.reduce((acc, metric) => {
		const score = metric.status === 'excellent' ? 4 : metric.status === 'good' ? 3 : metric.status === 'warning' ? 2 : 1;
		return acc + score;
	}, 0) / metrics.length;

	const overallStatus = overallScore >= 3.5 ? 'excellent' : overallScore >= 2.5 ? 'good' : overallScore >= 1.5 ? 'warning' : 'poor';
			...
			<CardContent>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					{metrics.map((metric) => {
						...
						return (
							<div key={metric.id} className="p-4 border rounded-lg bg-background/50">
