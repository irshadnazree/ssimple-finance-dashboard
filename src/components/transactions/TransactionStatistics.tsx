...
export function TransactionStatistics({ transactions, loading = false }: TransactionStatisticsProps) {
  const statistics = useMemo(() => {
    ...
    const largestTransaction = transactions.length > 0 ?
      Math.max(...transactions.map(t => Math.abs(t.amount))) : 0;
          ...
          <CardContent>
            {statistics.categoryStats.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                <span className="font-mono text-sm">No category data available</span>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {Object.entries(statistics.statusStats).map(([status, count]) => {
                const percentage = statistics.transactionCount > 0 ? 
                  (count / statistics.transactionCount) * 100 : 0;
                
                const statusConfig = {
                  completed: { icon: '✅', color: 'text-green-600', bgColor: 'bg-green-100' },
                  pending: { icon: '⏳', color: 'text-yellow-600', bgColor: 'bg-yellow-100' },
                  failed: { icon: '❌', color: 'text-red-600', bgColor: 'bg-red-100' },
                  cancelled: { icon: '🚫', color: 'text-gray-600', bgColor: 'bg-gray-100' }
                }[status] || { icon: '❓', color: 'text-gray-600', bgColor: 'bg-gray-100' };
                  ...
                  <div key={status} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-sm capitalize">{status}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-xs">
                          {count} txns
