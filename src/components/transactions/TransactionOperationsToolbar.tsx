...
export function TransactionOperationsToolbar({
  ...
  const [bulkStatus, setBulkStatus] = useState<Transaction['status']>('completed');
      ...
      <CardContent className="pt-0">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <Label htmlFor="bulkAction" className="font-mono text-xs uppercase tracking-wider">
                Bulk Action
              </Label>
              <Select
                id="bulkAction"
            ...
            {bulkAction === 'status' && (
              <div>
                <Label htmlFor="bulkStatus" className="font-mono text-xs uppercase tracking-wider">
                  New Status
                </Label>
                <Select
                  id="bulkStatus"
            ...
            {bulkAction === 'category' && (
              <div>
                <Label htmlFor="bulkCategory" className="font-mono text-xs uppercase tracking-wider">
                  New Category
                </Label>
                <Select
                  id="bulkCategory"
          ...
          <div className="w-full lg:w-auto">
            <Button
              ...
              disabled={!hasSelection || !bulkAction || loading || 
        ...
        {hasSelection && (
          <div className="mt-4 p-3 bg-muted/50 rounded-lg">
            <div className="flex flex-wrap gap-4 text-xs font-mono text-muted-foreground">
              ...
              <span>
                TOTAL: ${selectedTransactions.reduce((sum, t) => sum + Math.abs(t.amount), 0).toFixed(2)}
              </span>
              <span>
                TYPES: {Array.from(new Set(selectedTransactions.map(t => t.type))).join(', ')}
              </span>
            </div>
          </div>
        )}
