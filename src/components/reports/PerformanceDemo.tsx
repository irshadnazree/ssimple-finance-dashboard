...
export function PerformanceDemo({ transactions }: PerformanceDemoProps) {
	...
	const { result: cachedResult, isComputing } = useComputationCache(
	...
	const runQueryDemo = async () => {
		...
		setResults({
			queryDemo: {
				...
				optimizationRatio: (
					((transactions.length - filtered.length) / transactions.length) *
					100
				).toFixed(2),
			},
		});
	...
	const runLazyDemo = async () => {
		...
		setResults({
			lazyDemo: {
				...
				loadProgress: (((batchSize * 3) / transactions.length) * 100).toFixed(
					2,
				),
			},
		});
	...
	return (
		<Card className="w-full">
			<CardHeader>
				...
				<CardDescription>
					Interactive demonstration of caching, lazy loading, and query
								...
								<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
									<div className="p-4 bg-blue-50 rounded-lg">
										...
										<p className="text-2xl font-bold text-blue-700">
											{(
												(results.cacheDemo as Record<string, number>)
													.cacheMissTime || 0
											).toFixed(3)}
											ms
										</p>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									<div className="p-4 bg-orange-50 rounded-lg">
										...
										<p className="text-2xl font-bold text-orange-700">
											{(results.queryDemo as Record<string, number>)
												.originalCount || 0}
										</p>
									</div>
									<div className="p-4 bg-blue-50 rounded-lg">
									...
									<div className="p-4 bg-purple-50 rounded-lg">
										...
										<p className="text-2xl font-bold text-purple-700">
											{(results.queryDemo as Record<string, string>)
												.optimizationRatio || "0"}
											%
										</p>
									</div>
						...
						<div className="space-y-4">
							<div className="flex items-center justify-between">
								...
								<div className="flex gap-2">
									...
									<Button
										onClick={loadMore}
										disabled={!hasMore || isLazyLoading}
									>
							...
							{results.lazyDemo ? (
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									<div className="p-4 bg-blue-50 rounded-lg">
										...
										<p className="text-2xl font-bold text-blue-700">
											{(results.lazyDemo as Record<string, number>)
												.batch1Size || 0}
										</p>
									</div>
									<div className="p-4 bg-green-50 rounded-lg">
									...
									<div className="p-4 bg-orange-50 rounded-lg">
										...
										<p className="text-2xl font-bold text-orange-700">
											{(results.lazyDemo as Record<string, string>)
												.loadProgress || "0"}
											%
										</p>
									</div>
								...
								<div className="mt-4 p-4 bg-gray-50 rounded-lg">
									...
									<div className="grid grid-cols-2 gap-4 text-sm">
										<div>
											...
											<span className="ml-2">
												{(
													results.performanceDemo as Record<string, number>
