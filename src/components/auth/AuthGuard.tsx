...
export function AuthGuard({
  ...
  const { authState, getAuthState, addEventListener, removeEventListener } =
    useAuthStore();
  ...
  const checkAuthStatus = useCallback(async () => {
    try {
      ...
      if (
        requireAuth &&
        (currentState.status !== "authenticated" || !currentSession?.isActive)
      ) {
  ...
  }, [requireAuth, addEventListener, removeEventListener, checkAuthStatus]);
...
export function withAuthGuard<P extends object>(
  ...
  return function AuthGuardedComponent(props: P) {
    return (
      <AuthGuard requireAuth={options.requireAuth} fallback={options.fallback}>
        <Component {...props} />
      </AuthGuard>
    );
  };
...
export function useAuthGuard() {
  const { getAuthState, addEventListener, removeEventListener } =
    useAuthStore();
