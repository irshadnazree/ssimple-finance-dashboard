...
import { ArrowLeft, Fingerprint, Shield, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
export function BiometricSetup({ onResult, onBack }: BiometricSetupProps) {
  ...
  const checkBiometricCapabilities = async () => {
    try {
      ...
      if (!caps.enrolled) {
        setError('No biometric credentials are enrolled on this device. Please set up biometric authentication in your system settings first.');
        return;
      }
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <CardDescription>
              ...
              {step === 'consent' && `Set up ${getBiometricTypeLabel().toLowerCase()} authentication for secure access`}
              {step === 'setup' && 'Setting up biometric authentication...'}
              {step === 'complete' && 'Biometric authentication setup complete!'}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {step === 'check' && isLoading && (
          <div className="flex items-center justify-center p-8">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-muted-foreground">Checking device capabilities...</span>
            </div>
          </div>
        )}
        ...
        {step === 'consent' && capabilities && (
          <div className="space-y-6">
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-medium">{getBiometricTypeLabel()} Authentication</h3>
                <p className="text-sm text-muted-foreground mt-2">
                  Use your {getBiometricTypeLabel().toLowerCase()} to quickly and securely access your financial dashboard.
                </p>
              </div>
            </div>
        ...
        {step === 'setup' && (
          <div className="text-center space-y-4 p-8">
            ...
            <div>
              <h3 className="text-lg font-medium">Setting up {getBiometricTypeLabel()}...</h3>
              <p className="text-sm text-muted-foreground mt-2">
                Please follow the prompts from your device to complete the setup.
              </p>
            </div>
          </div>
        )}
        ...
        {step === 'complete' && (
          <div className="text-center space-y-4 p-8">
            ...
            <div>
              <h3 className="text-lg font-medium text-green-600">Setup Complete!</h3>
              <p className="text-sm text-muted-foreground mt-2">
                {getBiometricTypeLabel()} authentication has been successfully configured.
              </p>
            </div>
          </div>
        )}
