...
export function BiometricLogin({
	...
	const checkBiometricCapabilities = async () => {
		try {
			...
			if (!caps.enrolled) {
				setError(
					"No biometric credentials are enrolled. Please set up biometric authentication in your system settings.",
				);
				return;
			}
		} catch (err) {
		...
		try {
			...
			if (result.success) {
				...
				if (newAttemptCount >= maxAttempts) {
					setError(
						"Too many failed attempts. Please use PIN authentication or try again later.",
					);
				} else {
					setError(
	...
	const canAuthenticate =
		capabilities?.available && !isBlocked && state !== "authenticating";
			...
			<CardHeader>
				<div className="flex items-center space-x-3">
					...
					<div>
						...
						<CardDescription>
							{state === "ready" &&
								`Use your ${getBiometricTypeLabel().toLowerCase()} to sign in`}
						...
						{state === "ready" && canAuthenticate && (
							<>
								...
								<p className="text-sm text-muted-foreground">
									Place your {getBiometricTypeLabel().toLowerCase()} on the
									sensor or click the icon above
								</p>
							</>
						)}
				...
				{capabilities?.available && !isBlocked && (
					<div className="text-center">
						<p className="text-xs text-muted-foreground">
							Having trouble? Make sure your{" "}
							{getBiometricTypeLabel().toLowerCase()} is clean and properly
							positioned.
						</p>
					</div>
				)}
